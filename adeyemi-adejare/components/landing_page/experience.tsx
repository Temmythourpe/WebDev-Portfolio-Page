export default function Experience() {
  ...
  return (
    <section id="experience">
      <div className="py-20">
        <h1 className="text-4xl font-semibold text-[#344054] text-center mb-16">
          My <span className="text-primary">Work Experience</span>
        </h1>
        <div className="relative w-9/12 mx-auto">
          {/* Vertical dotted line */}
          <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-0.5 border-l-2 border-dashed border-gray-300"></div>
          ...
          {timeline.map((time, index) => (
            <div key={index} className="relative grid grid-cols-2 gap-8 mb-12">
              ...
              <div className="pr-8 text-left">
                ...
                <p className="text-[16px] text-gray-500">{time.location}</p>
                <p className="text-[12px] text-gray-500">{time.duration}</p>
              </div>

              {/* Center Dot */}
              <div className="absolute z-10 transform -translate-x-1/2 left-1/2 top-2">
                <div
                  className={`w-4 h-4 rounded-full ${time.color} border-4 border-white shadow-md`}
                />
              </div>
              ...
              <div className="pl-8 text-left">
                ...
                <p className="mt-1 text-sm text-gray-500">{time.description}</p>
              </div>
            </div>
          ))}
        </div>
